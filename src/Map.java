			...
			for (int j1 = 0; j1 < width; j1++) {
				for (int i2 = 0; i2 < height; i2++) {
					for (int j2 = 0; j2 < width; j2++) {
						...
						/*
						 * double heurD = -1; if (di >= dj) { heurD = dj*Math.sqrt(2) + di-dj; } else { // dj > di heurD = di*Math.sqrt(2) + dj-di; }
	...
	public DigraphW getGraph() {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				...
				if (j - 1 >= 0 && !getObstacle()[i][j - 1]) { // left
					int v = getIndex(i, j - 1);
					graph.addEdge(u, v, 1.0);
				}
				if (j + 1 < width && !getObstacle()[i][j + 1]) { // right
					int v = getIndex(i, j + 1);
					graph.addEdge(u, v, 1.0);
				}
				if (i - 1 >= 0 && !getObstacle()[i - 1][j]) { // up
					int v = getIndex(i - 1, j);
					graph.addEdge(u, v, 1.0);
				}
				if (i + 1 < height && !getObstacle()[i + 1][j]) { // down
					int v = getIndex(i + 1, j);
					graph.addEdge(u, v, 1.0);
				}

				double sqrt2 = Math.sqrt(2);
				if (j - 1 >= 0 && i - 1 >= 0 && !getObstacle()[i - 1][j - 1] && !getObstacle()[i][j - 1] && !getObstacle()[i - 1][j]) { // up-left
					int v = getIndex(i - 1, j - 1);
					graph.addEdge(u, v, sqrt2);
				}
				if (j + 1 < width && i - 1 >= 0 && !getObstacle()[i - 1][j + 1] && !getObstacle()[i][j + 1] && !getObstacle()[i - 1][j]) { // up-right
					int v = getIndex(i - 1, j + 1);
					graph.addEdge(u, v, sqrt2);
				}
				if (j - 1 >= 0 && i + 1 < height && !getObstacle()[i + 1][j - 1] && !getObstacle()[i][j - 1] && !getObstacle()[i + 1][j]) { // down-left
					int v = getIndex(i + 1, j - 1);
					graph.addEdge(u, v, sqrt2);
				}
				if (j + 1 < width && i + 1 < height && !getObstacle()[i + 1][j + 1] && !getObstacle()[i][j + 1] && !getObstacle()[i + 1][j]) { // down-right
					int v = getIndex(i + 1, j + 1);
					graph.addEdge(u, v, sqrt2);
				}
		...
		/*
		 * if (graph.hasParallelEdges()) { System.err.println("getGraph: Graph has parallel edges."); System.exit(-1); } if (!graph.isSymmetric()) {
		 * System.err.println("getGraph: Graph is not symmetric."); System.exit(-1); }
		 */
